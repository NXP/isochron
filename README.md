...
```
[root@OpenIL]# for eth in eno2 eno3 swp0 swp1 swp2 swp3 swp4; do ip link set dev $eth up; done

[root@board1]# ./tsn-scripts/8021cb.sh -t 3 -i 1
[root@board2]# ./tsn-scripts/8021cb.sh -t 3 -i 2
[root@board3]# ./tsn-scripts/8021cb.sh -t 3 -i 3
```

Expected output from one board:

```
[root@LS1028ARDB ~] # ./tsn-scripts/8021cb.sh -t 3 -i 1
	......(ignore)
	tsntool cbrec --device swp0 --index 34 --seq_len 16 --his_len 31 --rtag_pop_en
	echo reply:swp0
	echo reply:0
	tsntool cbrec --device swp0 --index 35 --seq_len 16 --his_len 31 --rtag_pop_en
	echo reply:swp0
	echo reply:0
	Set the external device links to with vid swp2:104 swp3:107
	vlan set swp2:104
	vlan set swp3:107
	Adding VLAN mangling rules (see with 'tc filter show dev eno2 egress && tc filter show dev eno2 ingress')
	push/pop vlan 101 for eno2
	push/pop vlan 102 for eno2
	push/pop vlan 103 for eno2
	push/pop vlan 104 for eno2
	push/pop vlan 105 for eno2
	push/pop vlan 106 for eno2
	push/pop vlan 107 for eno2
	push/pop vlan 108 for eno2
	push/pop vlan 109 for eno2
	Populating the ARP table...
	arp:172.15.0.2:00:04:9f:63:35:18
	arp:172.15.0.3:00:04:9f:63:35:19
	arp:172.15.0.5:00:04:9f:63:35:21
	arp:172.15.0.6:00:04:9f:63:35:22
	arp:172.15.0.8:00:04:9f:63:35:24
	arp:172.15.0.9:00:04:9f:63:35:25
	Ready to send/receive traffic.
	IP address of eno2 is 172.15.0.1
	IP address links to swp2 is 172.15.0.4
	IP address links to swp3 is 172.15.0.7
```
