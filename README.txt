...
It works by performing hardware timestamping of custom (non-PTP) packets, and
therefore only works on hardware that can timestamp any packet and not just
PTP.

An isochron instance needs to be started at the sender side, and another one at
the receiver side. Optionally, the receiver can open a TCP socket through which
statistics are passed back to the sender, and the latency characteristics of
the link are determined.

Isochron (isochronous traffic == running at equal intervals) is intended to be
used in conjunction with IEEE 802.1Qbv networking equipment, and enqueue
traffic into the kernel, which will be sent as soon as its traffic class' time
gate opens. The operating principles of the isochron sender are detailed below.
...
Concentrating on a single time slot (isochron), let's overlap a few timelines
to understand where the time is spent:
...
The reason why isochron is scheduled yet once more, after the frame
transmission has occured, is to collect the TX timestamp and print the log
message. This is its non-critical portion.

Where:
t0, t7: wakeup times for the isochron user space process
t1: hand-over times between isochron process and kernel stack (i.e.
...
t4: kernel processes the TX timestamp and enqueues it back towards the
    application socket.
t5: second wakeup time for the isochron application, to collect the
...
At this time let's introduce the isochron command line arguments:

--cycle-time: The length of the MAC schedule in nanoseconds. Also the period
              of the isochron process.
